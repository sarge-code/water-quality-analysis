...
function safeProb(prob) {
  if (isNaN(prob) || prob === undefined || prob === null) return 0.5
  return Math.min(Math.max(prob, 0), 1)
}

// ── Decision Tree ──────────────────────────────────────────────────────────
export function predictDT(x, weights) {
  const prob = safeProb(traverseTree(weights.dt.tree_, x))
...
export function predictLR(x, weights) {
  ...
  const prob = safeProb(sigmoid(logit))
  return { prediction: prob >= 0.5 ? 1 : 0, probability: prob }
}
...
export function predictNB(x, weights) {
  ...
  const prob   = safeProb(sum > 0 ? exps[1] / sum : 0.5)
  return { prediction: prob >= 0.5 ? 1 : 0, probability: prob }
}
...
export function predictRF(x, weights) {
  ...
  const prob  = safeProb(probs.reduce((a, b) => a + b, 0) / probs.length)
  return { prediction: prob >= 0.5 ? 1 : 0, probability: prob }
}
...
export function predictGB(x, weights) {
  ...
  let score = Math.log((init_pred + 1e-10) / (1 - init_pred + 1e-10))

  trees.forEach(tree => {
    const val = traverseTree(tree, x)
    if (!isNaN(val)) {
      score += learning_rate * val
    }
  })

  const prob = safeProb(sigmoid(score))
  return { prediction: prob >= 0.5 ? 1 : 0, probability: prob }
}
...
export function predictXGB(x, weights) {
  ...
  let score = Math.log((base_score + 1e-10) / (1 - base_score + 1e-10))

  try {
    trees.forEach(treeStr => {
      const tree = JSON.parse(treeStr)
      const val  = traverseXGBTree(tree, x, features)
      if (!isNaN(val)) score += learning_rate * val
    })
  } catch (e) {
    console.warn('XGBoost tree traversal error:', e)
  }
...
export function predictKNN(x, weights) {
  ...
  const prob = safeProb((rf.probability + et.probability + dt.probability) / 3)
  return { prediction: prob >= 0.5 ? 1 : 0, probability: prob }
}
...
export function predictAll(inputs, weights) {
  ...
  return [
    ...
    return {
      ...
      probability: isNaN(result.probability) ? 50 : Math.round(result.probability * 100),
      ...metrics,
    }
  })
}